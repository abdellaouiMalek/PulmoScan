...
def analyze_class_distribution(processed_df):
    ...
    train_counts = processed_df[processed_df["split"] == "train"][
        "category"
    ].value_counts()
...
def apply_advanced_augmentation(image):
    ...
    if augmentation_type == "color_jitter":
        ...
        img_array = np.clip((img_array - mean) * contrast_factor + mean, 0, 255).astype(
            np.uint8
        )
...
def create_balanced_dataset(
    processed_df, augmentation_factors, output_dir="balanced_images"
):
    """
        ...
        for category in categories:
            ...
            for _, row in tqdm(
                ...
                image_path = os.path.join(
                    "processed_images", split, category, row["processed_filename"]
                )
                ...
                output_path = os.path.join(
                    output_dir, split, category, row["processed_filename"]
                )
                ...
                if split == "train":
                    ...
                    for i in range(factor - 1):
                        ...
                        output_path = os.path.join(
                            output_dir, split, category, output_filename
                        )
...
def analyze_balanced_dataset(balanced_df):
    ...
    comparison_df = pd.DataFrame(
        {"Original": orig_train_category_counts, "Balanced": train_category_counts}
    )
