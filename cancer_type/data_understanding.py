...
def load_data():
    ...
    try:
        ...
        print(
            f"  - Clinical data: {clinical.shape[0]} rows, {clinical.shape[1]} columns"
        )
        print(
            f"  - Pathology data: {pathology.shape[0]} rows, {pathology.shape[1]} columns"
        )
...
def analyze_histology_subtypes(merged_df):
    ...
    plt.figure(figsize=(14, 8))
    sns.countplot(
        y="roi_histology_subtype1", data=merged_df, order=subtype1_dist.index[:10]
    )
...
def prepare_data_for_modeling(merged_df):
    ...
    for col in modeling_df.columns:
        ...
        if missing > 0:
            print(
                f"Column {col} has {missing} missing values ({missing/len(modeling_df)*100:.2f}%)"
            )
    ...
    print(
        f"After dropping rows with missing values in key columns: {modeling_df.shape[0]} rows"
    )
