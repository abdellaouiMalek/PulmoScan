...
def create_directories(metadata=None):
    """Create necessary directories for processed data, including dynamic categories from metadata"""
    dirs = [
        'processed_images',
    ...
    # Add unique categories from metadata if provided
    if metadata is not None and 'major_category' in metadata.columns:
        unique_categories = metadata['major_category'].dropna().unique()
        categories.extend([cat for cat in unique_categories if cat not in categories and cat != 'Unknown'])
    ...
    for d in dirs:
        ...
        if d != 'processed_images':
            for category in categories:
                ...
                category_dir = os.path.join(d, category_dir_name)
...
def load_metadata():
    ...
    try:
        ...
        if os.path.exists('D:/PLUMOSCAN/PulmoScan/output/modeling_data.csv'):
            print("Loading prepared modeling data...")
            metadata = pd.read_csv('D:/PLUMOSCAN/PulmoScan/output/modeling_data.csv')
        else:
            # If not available, load and merge the raw data
            print("Prepared data not found. Loading and merging raw data...")
            clinical = pd.read_csv("D:/PLUMOSCAN/PulmoScan/data/lung_cancer.csv")
            pathology = pd.read_csv("D:/PLUMOSCAN/PulmoScan/data/Pathology_Images.csv")
            ...
            def map_to_major_category(cancer_type):
                ...
                for category, types in major_categories.items():
                    if any(t in str(cancer_type) for t in types):
                        return category
                return 'Other'
        ...
        required_columns = ['pid', 'major_category']
        filename_column = 'image_filename'  # Change this to the correct column name if different
        possible_filename_columns = ['image_filename', 'svs_filename', 'filename', 'image_path']
        ...
        # Find the column containing filenames
        ...
        if filename_column is None:
            raise KeyError(f"No filename column found in metadata. Expected one of: {possible_filename_columns}")
        
        # Rename the filename column to 'image_filename' for consistency
        if filename_column != 'image_filename':
            metadata = metadata.rename(columns={filename_column: 'image_filename'})
            print(f"Renamed column '{filename_column}' to 'image_filename'")

        # Standardize major_category values
        valid_categories = [
            'Adenocarcinoma',
            'Squamous_Cell_Carcinoma',
            'Neuroendocrine_Carcinoma',
        ...
        def standardize_category(category):
            ...
            if 'Squamous' in category and 'Cell' not in category:
                return 'Squamous_Cell_Carcinoma'
            return category if category in valid_categories else 'Other'

        metadata['major_category'] = metadata['major_category'].apply(standardize_category)
...
def process_slides(metadata, image_dirs, output_dir='processed_images', max_slides=None,
    ...
    Args:
        ...
        image_dirs: List of directories containing SVS files
    ...
    # Verify required columns
    required_columns = ['pid', 'image_filename', 'major_category']
    if not all(col in metadata.columns for col in required_columns):
        missing = [col for col in required_columns if col not in metadata.columns]
        raise KeyError(f"Missing required columns in metadata: {missing}")
    ...
    for _, row in tqdm(slides_to_process.iterrows(), total=len(slides_to_process)):
        ...
        # Search for the slide in all image directories
        slide_path = None
        ...
        if slide_path is None:
            print(f"Slide not found: {filename} in any of {image_dirs}")
            continue
        ...
        for i, (x, y) in enumerate(coords):
            ...
            output_category_dir = os.path.join(output_dir, split, category_dir_name)
            if not os.path.exists(output_category_dir):
                os.makedirs(output_category_dir)
                print(f"Created directory: {output_category_dir}")
...
def analyze_processed_data(processed_df):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join('processed_images', 'processed_category_distribution.png'))
    plt.close()
    ...
    plt.ylabel('Count')
    plt.tight_layout()
    plt.savefig(os.path.join('processed_images', 'processed_split_distribution.png'))
    plt.close()
    ...
    plt.xticks(rotation=45)
    plt.tight_layout()
    plt.savefig(os.path.join('processed_images', 'processed_category_split_distribution.png'))
    plt.close()
...
def main():
    ...
    # Load metadata first to get categories
    metadata = load_metadata()
    if metadata is None:
        print("Error: Could not load metadata. Exiting.")
        return

    # Create directories with dynamic categories from metadata
    ...
    valid_dirs = [d for d in image_dirs if os.path.exists(d)]
    if not valid_dirs:
        print(f"Error: None of the image directories exist: {image_dirs}")
        return
    else:
        print(f"Found {len(valid_dirs)} valid image directories: {valid_dirs}")

    # Process slides (process all slides)
    processed_df = process_slides(metadata, valid_dirs, max_slides=None)
